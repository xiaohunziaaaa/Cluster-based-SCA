...
int main(int argc, char *argv[]){
    ...
    //INT8 参数路径
    string weight_path="../data/weights_q.txt";
    ...
    string suffix="_"+to_string(INPUT_NUM)+"_"+to_string(noise)+".txt";
    ...
    int* weight_vec=new int[W_NUMBER];
    ifstream weights_reader;
    weights_reader.open(weight_path);
    string temp_weight; int weight_counter=0;
    while(getline(weights_reader,temp_weight)){
        weight_vec[weight_counter++]=atoi(temp_weight.c_str());
    }
    ...
    for(int i=0;i<W_NUMBER;i++){
        printf("Total weights are %d,now guessing weight No.%d\r",W_NUMBER,i+1);
        ...
        for(int j=0;j<INPUT_NUM;j++){
            ...
            power_hm_vec[j]=hmming(act_vec[j],noise);
            power_random_vec[j]=randomleak(act_vec[j],noise);
